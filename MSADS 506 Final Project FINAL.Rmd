...
```{r}
df <- read.csv("C:/Users/lvand/OneDrive/Desktop/Time Series Analysis/FINAL GROUP PROJECT/baggagecomplaints.csv")
...
boxplot(df$Baggage, main="Baggage Complaints")
boxplot(df$Scheduled, main="Scheduled Flights")
boxplot(df$Cancelled, main="Canceled Flights")
boxplot(df$Enplaned, main="Enplaned Flights")
```
...
```
The  moving average does not capture the seasonality in the data. Seasons with high baggage complaints are under-forecasted, and seasons with low baggage complaints are over-forecasted.
...
HW_data <- data.frame(
  Method = c("American Eagle", "United", "Hawaiian"),
  ME = c(45.45592, -1795.942, 653.7532),
  RMSE = c(1864.611, 3408.754, 873.2616),
  MAE = c(1706.882, 2373.28, 739.4063),
  MPE = c(2.874442, -11.55448, 27.89336),
  MAPE = c(18.38551, 18.27006, 34.23057),
  ACF1 = c(-0.16718, -0.3307423, 0.3953068),
  Theils_U = c(1.151863, 1.190425, 1.841316)
)

# Printing the table using kable
#knitr::kable(data, caption = "Accuracy Metrics for Different Methods")
cat(kable(HW_data, caption = "Holt-Winters Accuracy Metrics for Each Airline", format = "markdown"),
    sep = "\n")
```
