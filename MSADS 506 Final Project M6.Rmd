...
bag.nnetar <- nnetar(training.ts, repeats = 20, p = 11, P = 1, size = 6)
...
accuracy_table <- data.frame(
  ...
  ME = c(-432.5183, 45.45592, 850.5374, 946.7145 ),
  RMSE = c(1262.605, 1864.611, 2047.311, 1440.186),
  MAE = c(1061.951, 1706.882, 1768.298, 1122.739),
  MPE = c(-6.236093, 2.874442, 11.61096, 9.226714),
  MAPE = c(11.74719, 18.38551, 20.11946, 11.29808),
  ACF1 = c(-0.1278269, -0.16718, -0.1133736, -0.05217826),
  Theils_U = c(0.8029777, 1.151863, 1.264255, 0.9141654)
)
...
ha.nnetar <- nnetar(HA_training.ts, repeats = 20, p = 11, P = 1, size = 6)
...
HA_data <- data.frame(
  ...
  ME = c(653.7532, 506.2353, 658.8911, 431.7049233),
  RMSE = c(873.2616, 724.8699, 820.6329, 605.89102),
  MAE = c(739.4063, 635.6922, 658.8911, 488.810018),
  MPE = c(27.89336, 20.56007, 29.53305, 17.97379798),
  MAPE = c(34.23057, 30.24288, 29.53305, 22.4247017 ),
  ACF1 = c(0.3953068, 0.3674934, 0.318373, 1.19416128),
  Theils_U = c(1.841316, 1.53408, 1.671758, 0.2544057  )
)
...
Among the given models, the Neural Network Autoregression seems to perform relatively better in terms of lower errors (ME, RMSE, MAE) and lower percentage errors (MPE, MAPE), although it exhibits higher autocorrelation in residuals. The STL + ARIMA model also shows promising results with moderate errors and correlation, while Holt-Winter and Simple Exponential Smoothing models perform comparably with higher errors and moderate correlation.
